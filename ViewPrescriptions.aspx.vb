Public Class ViewPrescriptions
    ...
    Private Sub LoadData()
        ...
        With Me
            ...
            adataset = DBConnection.ViewPrescription
            .grdStudents.DataSource = adataset.Tables(0)
            .grdStudents.DataBind()
            If Cache("PrescriptionViewData") Is Nothing Then
                Cache.Add("PrescriptionViewData", New DataView(adataset.Tables(0)), Nothing, Caching.Cache.NoAbsoluteExpiration, System.TimeSpan.FromMinutes(10), Caching.CacheItemPriority.Default, Nothing)
            End If
        End With
    End Sub
    ...
    Public Sub NewPrescription()
        Dim DBConnection As New DBConnection
        Try
            Dim sb As New StringBuilder
            sb.Append("<script language='javascript'>")
            sb.Append("window.open('AddPrescription.aspx),")
    ...
    Public Sub SearchPrescription()
        Dim DBConnection As New DBConnection
        Try
            Dim sb As New StringBuilder
            sb.Append("<script language='javascript'>")
            sb.Append("window.open('SearchPrescription.aspx),")
            sb.Append("'width=525, height=525, menubar=no, resizable=yes, left=50, top=50, scrollbars=yes');")
            sb.Append("</script>")
            ClientScript.RegisterClientScriptBlock(GetType(Page), "PopupScript", sb.ToString())
        Catch ex As Exception
    ...
    Private Sub SortRecords(ByVal sortExpress As String)
        ...
        Dim lastValue, theSortField, theDirection, wildChar, sortExpression As String
        ...
        Try
            ...
            Else
                Me.sortDir = "desc"
            End If
            source = Cache("PrescriptionViewData")
