Public Class ViewPhysicians
    ...
    Private Sub LoadData()
        ...
        With Me
            ...
            If Cache("PhysicianViewData") Is Nothing Then
                Cache.Add("PhysicianViewData", New DataView(adataset.Tables(0)), Nothing, Caching.Cache.NoAbsoluteExpiration, System.TimeSpan.FromMinutes(10), Caching.CacheItemPriority.Default, Nothing)
            End If
        End With
    End Sub
    ...
    Public Sub NewPhysician()
        Dim DBConnection As New DBConnection
        Try
            Dim sb As New StringBuilder
            sb.Append("<script language='javascript'>")
            sb.Append("window.open('AddPhysician.aspx),")
    ...
    Public Sub SearchPhysician()
        Dim DBConnection As New DBConnection
        Try
            Dim sb As New StringBuilder
            sb.Append("<script language='javascript'>")
            sb.Append("window.open('SearchPhysician.aspx),")
            sb.Append("'width=525, height=525, menubar=no, resizable=yes, left=50, top=50, scrollbars=yes');")
            sb.Append("</script>")
            ClientScript.RegisterClientScriptBlock(GetType(Page), "PopupScript", sb.ToString())
        Catch ex As Exception
    ...
    Private Sub SortRecords(ByVal sortExpress As String)
        ...
        Try
            ...
            Else
                Me.sortDir = "desc"
            End If
            source = Cache("PhysicianViewData")
